...
1) Список (list) 
    - изменяемость (+), индексируемость (+), уникальность (-), создаём -> [], list()
2) Кортеж (tuple) 
    - изменяемость (-), индексируемость (+), уникальность (-), создаём -> (), tuple()
3) Строка (string) 
    - изменяемость (-), индексируемость (+), уникальность (-), создаём -> '', ""
4) Множество (set) 
    - изменяемость (+), индексируемость (-), уникальность (+), создаём -> {elm1, elm2}, set()
5) Неизменное множество (frozenset)
    - изменяемость (+), индексируемость (-), уникальность (+), создаём -> frozenset()
6) Словари (dict)
    - изменяемость (+), индексируемость (-), уникальность (+), создаём -> frozenset()
"""
