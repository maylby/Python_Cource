...
# out = [] # вариант 1, переменная (res) принимает '[]' значения списка (data)
res = list() # вариант 2, переменной 'res' передан '=' список 'list()',
             # принимающий значения (data) 
for i in data : # пербор для (for) элементов (i) в (in) списке (data)
    if i % 2 == 0: # если (if) число (i) чётное (делится на 2), то заносим его в 'res'
        res.append((i, i ** 2)) # в 'res' добавляем (append) квадрат числа (i),
